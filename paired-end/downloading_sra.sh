...
pigz "${OUTDIR}/${SAMPLE}_1.fastq"
pigz "${OUTDIR}/${SAMPLE}_2.fastq"

# remove the prefetched .sra file to save space
rm -rf "$SAMPLE"

# check if files were created successfully
if [[ ! -f "${OUTDIR}/${SAMPLE}._1fastq.gz" ]]; then
    echo "ERROR: Failed to create ${OUTDIR}/${SAMPLE}_1.fastq.gz"
    exit 1
fi

if [[ ! -f "${OUTDIR}/${SAMPLE}._2fastq.gz" ]]; then
    echo "ERROR: Failed to create ${OUTDIR}/${SAMPLE}_2.fastq.gz"
    exit 1
fi
